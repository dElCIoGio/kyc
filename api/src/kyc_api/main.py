...
def _install_error_handlers(application: FastAPI) -> None:
    ...
    async def session_not_found(_request: Request, _exc: SessionNotFound) -> JSONResponse:
        return _error_response(404, "SESSION_NOT_FOUND", "Session was not found")

    @application.exception_handler(SessionConflict)
    async def session_conflict(_request: Request, exc: SessionConflict) -> JSONResponse:
    ...
    async def session_capacity(
        _request: Request, _exc: SessionCapacityExceeded
    ) -> JSONResponse:
    ...
    async def job_capacity(_request: Request, _exc: JobCapacityExceeded) -> JSONResponse:
    ...
    async def session_error(_request: Request, _exc: SessionStoreError) -> JSONResponse:
